...
clues(
   ...
   [
     /* En lugar de invocar las pistas una por una como se muestra abajo,
        es mejor usar la relación pistas/2; la cual recibe una lista con
        los números de cada pista que se desea invocar.
        De esa manera se puede controlar más fácilmente cuáles
        pistas se quieren probar y en qué orden.
        Se tiene que cambiar las pistas de "pista1(Viajes)" a "pista(1,Viajes)".
     */
     pistas([1,2,3,4,5],Viajes)
...
queries(
        ...
        [ 
          % Pregunta ¿Quién cubrió las olimipiadas?
...
% Esta relación invoca las pistas que se indican
...
pista(1,E) :- member(viaje(7,_,boda),E).
...
pista(2,E) :- select(viaje(5,_,_),E,E2),
...
pista(3,E) :- select(viaje(D2,_,eleccion),E,E2),
